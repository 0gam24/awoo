...
## 9-1. 리포트 CSV 적재 (주 1회, 2026-09-16 신설)

**왜 필요한가.** 2026-09-15부터 순위 측정은 공식 웹문서 검색 API만 쓴다(통합검색 페이지는 robots.txt가 봇을 막는다). API 순위는 "색인 안의 자리"라 **사람이 보는 화면 순위와 다르다** — 실측 8건에서 화면 3위가 API 13위였다. 화면에서의 노출·클릭·평균순위는 **여기 리포트가 유일한 진실값**이다.

1. https://searchadvisor.naver.com → 웹마스터 도구 → `awoo.or.kr` → **리포트**(검색어 탭·문서 탭) → 기간을 지난 7일로 → CSV 내려받기
2. 받은 파일을 `docs/ops/inbox/`에 넣는다
3. ```bash
   npm run advisor:import
   ```
4. 산출 `src/data/search-advisor/search-advisor-<기간>.json` — `queries[]`(검색어·노출·클릭·평균순위), `docs[]`(URL별)

- 같은 검색어가 날짜별로 쪼개져 있으면 노출·클릭은 더하고 평균순위는 노출 가중평균으로 모은다.
- **열 이름을 못 알아보면 그 파일을 건너뛰고 헤더를 그대로 보여준다.** 아무 열이나 순위로 넣으면 판정이 조용히 틀어지기 때문이다. 그때는 `scripts/ingest-search-advisor.mjs`의 `KEYS`에 이름을 한 줄 더한다.
- 아직 **실제 네이버 리포트 CSV로 돌려본 적이 없다**(합성 파일로만 검증). 첫 회차에 열 이름이 맞는지 확인하라.
