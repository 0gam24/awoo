...
| 필드 | 규칙 | 게이트 |
|---|---|---|
| `title` | 한 문장, 접미사 ` \| 지원금가이드` 포함 시 40자 이하 권장. 클릭베이트 금지. **롱테일 우선(§5-네이버): 브로드 헤드 대신 `지역명+지원금+대상자/조건`으로 구체화**. **긴 줄표(—/–) 금지 → 콤마·자연스러운 구(§5-AI티, 애드센스)** | lint warn(40자 초과) / **err(긴 줄표, 2026-07-11+)** |
| `slug` | 영숫자·하이픈만. 파일명=slug. 날짜 접미사 `-YYYY-MM-DD` 유지(4중 일치) | 기존 |
| **URL(발행 링크)** | **2026-07-10+ 발행분은 날짜 제거** — `/issues/{slug에서 -YYYY-MM-DD 제거}/` (구글 SEO, 운영자 2026-07-09 지시). 그 이전 발행분은 `/issues/{date}/{slug}/` 유지. `src/lib/issue-url.mjs` 자동 파생 | URL slug 예약어/전역중복 시 **err** |
...
### AI 티 제거 (2026-07-10 운영자 지시 — 애드센스 재승인 필수)
사이트가 "AI로 대충 찍어낸 곳"으로 보이면 구글·애드센스 심사에 치명적. **사람이 쓴 것처럼 자연스럽게.** 이미 발행된 글은 제목 변경 시 SEO 손해라 건드리지 않고 **신규(2026-07-11+)만 적용**.
- **긴 줄표(— em-dash / – en-dash) 금지 (lint err)**: 제목에서 `—` 대신 콤마·자연스러운 구. ❌ "교육비 세액공제 2026 — 자녀 300만" → ✅ "교육비 세액공제 2026, 자녀 300만 대학 900만". 본문·metaDescription도 가급적 회피.
- **정형 말버릇 반복 금지 (lint warn)**: "핵심은 세 가지입니다", "정리하면", "결론부터 말하면" 등을 글마다 반복하면 AI 티. 리드·문단 도입부를 매 글 다르게.

### 네이버
- **롱테일 타겟 우선 (2026-07-09 운영자 지시 — 신규 도메인 핵심 전략)**: 브로드 헤드 키워드("정부 지원금"·"청년 월세 지원")는 경쟁 과포화라 신규 도메인이 상위 노출 불가. **`지역명 + 지원금 + 대상자/조건` 롱테일**로 제목·tags·metaDescription을 좁힌다.
  - ❌ "청년 월세 지원" → ✅ "파주시 청년 월세 지원 대상자·소득기준 2026"
  - ❌ "정부 지원금" → ✅ "인천 미추홀구 기초수급자 난방비 신청 대상"
  - 지역 적용 가능 주제(월세·난방비·청년수당·출산·지자체 사업)는 **지역명 필수**. 지역 무관 전국 정책은 대상자+수치+조건으로 좁힌다("월 519만원 국민연금 감액 대상 재직자").
  - tags에도 지역명+대상자 롱테일 3개 이상 포함. `/subsidies/category/[category]/persona/[persona]` 4축 hub와 내부링크 연계.
