...
- **`/today`(오늘 포스팅) = 완전 자율 모드** (2026-06-12 운영자 지시): GATE-C·GATE-D 자동 결재 — fact-checker PASS + quality-gate PASS + lint/build 통과 = 즉시 발행. 운영자 입력 0회. 발행 후 "취소" 시 revert 롤백.
- `/post`·`/sprint` 직접 호출 시에는 기존 GATE-C(각도 확정)·GATE-D(발행 결재) 유지.
- 발행 후 GSC·Naver 색인 요청 1클릭은 항상 운영자 몫 (리포트에 링크 출력).

## 3. 일일 운영 루틴 (최소 입력)

```
아침: "오늘 포스팅"     ← 이 한마디가 전부. 점검 → 발굴 → 작성 → 검증 → 발행 완주 (입력 0회)
  → 끝나면 색인 요청 링크만 클릭 (3분)

개별 실행 (수동 게이트 유지): /post 키워드 · /sprint 3 · /traffic
```
