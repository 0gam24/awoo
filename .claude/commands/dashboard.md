...
## "눈확인:" 메시지 처리 (2026-09-15)

위젯 상위 5건의 [첫 화면][한 번 스크롤][그 아래] 버튼은 `눈확인: "<쿼리>" = <값>` 을 보낸다. 웹문서 묶음이 통합검색 화면 어디쯤 있는지는 공식 API로 못 재서 운영자 눈으로 받는 값이다(종전 `webDocOffset` 대체).
```bash
node scripts/eye-offset.mjs --query="<쿼리>" --value="<첫 화면|한 번 스크롤|그 아래>"
node scripts/keyword-pipeline.mjs --serp-replay
```
그다음 위젯만 다시 띄운다(6단계). `그 아래`면 그 후보는 닫힘으로 빠진다. 기록은 `docs/ops/eye-offset.json`. **네이버 화면을 Claude가 대신 열어 보지 않는다.**
